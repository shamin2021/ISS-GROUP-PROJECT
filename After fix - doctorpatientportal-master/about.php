...
<?php
	...
	if(isset($_POST['btn_login'])){
		...
		$stmt1 = $con->prepare('SELECT * FROM admin WHERE email = ?');
		$stmt2 = $con->prepare('SELECT * FROM patient WHERE email = ?');
		$stmt3 = $con->prepare('SELECT * FROM doctor WHERE email = ?');
...
<noscript>
  ...
  <head>
	...
	<meta http-equiv="Content-Security-Policy" 
		content="default-src 'self'; img-src 'self' https://img.nopcommerce.com; 
...
.navbar-inverse .navbar-nav > .open > a:hover{
 ...
 .tt td{
  ...
  <body>
	...
	$st1 = $con->prepare("SELECT * FROM doctor where email = ?");
	$st2 = $con->prepare("SELECT * FROM patient where email = ?");
	$st3 = $con->prepare("SELECT * FROM admin where email = ?");

	$st1->bind_param('s', $eml);
	$st1->execute();
	$rst1 = $st1->get_result();

	$st2->bind_param('s', $eml);
	$st2->execute();
	$rst2 = $st2->get_result();

	$st3->bind_param('s', $eml);
	$st3->execute();
	$rst3 = $st3->get_result();
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
	  ...
	  <br>
        ...
        <center>
	<input type="hidden" name="csrd_token" value="<?php echo $token; ?>" >	
	<input type="submit" value="Login" name="btn_login" class="btn btn-success"></center>
      </div>
		</form>
      </div>
    </div>
     ...
     <div class="modal-content">
	  ...
	  <div class="modal-body">
	   ...
	   <input type="radio" value="Female" name="pgender" required> Female <br></br>
		...
		<center>
	<input type="hidden" name="csrd_token" value="<?php echo $token; ?>" >	
	<input type="submit" value="Register" name="btn_pat" class="btn btn-danger">
	   ...
	   <input type="text" name="docid" class="form-control" required placeholder="Enter your Doctor ID"><br>
		...
		<center>
	<input type="hidden" name="csrd_token" value="<?php echo $token; ?>" >	
	<input type="submit" value="Register" name="btn_doc" class="btn btn-danger">
